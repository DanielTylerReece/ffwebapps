...
impl Runtime {
    ...
    pub fn link(&self) -> Result<()> {
        ...
        // `uninstall` removed the runtime directory; recreate it before copying
        // or symlinking entries into it (otherwise the first non-"defaults" entry
        // fails with ENOENT, depending on `read_dir` ordering).
            ...
            for entry in read_dir(FFOX)?.flatten() {
                ...
                match entry.file_name().expect("Couldn't retrieve a file name").to_str() {
                    ...
                    Some("defaults") => {
                        ...
                        // `entry` is already `<ffox>/defaults`, so the source is
                        // `pref/channel-prefs.js` (not `defaults/pref/...`).
                        symlink(
                            entry.join("pref/channel-prefs.js"),
